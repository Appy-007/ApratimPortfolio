  ...
  return (
    <>
      ...
      <div>
        <ul
          ref={sidebarRef}
          className="flex md:hidden flex-col gap-4 h-screen bg-rose-50 dark:bg-darkTheme px-10 py-20 fixed -right-64 w-65 z-50 top-0 bottom-0  transition duration-300"
        >
          <button className="absolute top-3 right-3" onClick={closeMobileNav}>
            <Image
              src={isLightMode ? assets.close_black : assets.close_white}
              alt="mobile_screen_nav_close"
              className="w-3"
            />
          </button>
